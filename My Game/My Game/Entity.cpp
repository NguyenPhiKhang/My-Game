...
{
	// If either entity is not active then no collision may occur
	if (!active || !ent.getActive())
		return false;

	// Check for collision using Asis-Aligned Bounding Box
...
//       sets collisionVector if collision
//=============================================================================
bool Entity::collideCornerCircle(D3DXVECTOR2 corner, Entity& ent, D3DXVECTOR2& collisionVector)
{
...
void Entity::computeRotatedBox()
{
	...
	D3DXVECTOR2 rotatedX(cos(spriteData.angle), sin(spriteData.angle));
	D3DXVECTOR2 rotatedY(-sin(spriteData.angle), cos(spriteData.angle));

	const D3DXVECTOR2* center = getCenter();
